...
class OrderPage(BasePage):
    ...
    def fill_in_input_name(self, name):
        return self.add_text_to_element(OrderPageLocators.input_name_field, name)

    @allure.step('Заполняем поле Фамилия')
    def fill_in_input_last_name(self, last_name):
        return self.add_text_to_element(OrderPageLocators.input_last_name_field, last_name)

    @allure.step('Заполняем поле Адресс')
    def fill_in_input_address(self, address):
        return self.add_text_to_element(OrderPageLocators.input_address_field, address)

    @allure.step('Заполняем поле Метро')
    def fill_in_input_metro(self, metro):
        return self.add_text_to_element(OrderPageLocators.input_metro_field, metro)

    @allure.step('Выбираем метро из списка')
    def click_from_element_list_metro(self, metro):
        locator_metro = self.format_locators(OrderPageLocators.metro_last_element_button, metro)
        return self.click_to_element(locator_metro)

    @allure.step('Заполняем поле Телефон')
    def fill_in_input_phone(self, phone):
        return self.add_text_to_element(OrderPageLocators.input_phone_field, phone)
    ...
    def click_button_order_finall(self):
        self.click_to_element(OrderPageLocators.button_final_order)
    ...
    def check_accept_order(self):
        return self.get_text_from_element(OrderPageLocators.title_add_order)
